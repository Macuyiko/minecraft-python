...
public class PyLinkUtils {
	...
	public static void unpack(String destDir, String prefix) {
		...
		try (JarFile jar = new JarFile(
				PyLinkUtils.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath())) {
	...
	public static void addURLToLibraryClassLoader(ClassLoader loader, URL u) {
		...
		try {
			...
			try (URLClassLoader libraryloader = (URLClassLoader) libraryloaderfield.get(loader)) {
				URL[] urls = new URL[] {};
